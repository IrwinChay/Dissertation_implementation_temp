...
class TensorizedSMPC(nn.Module):
    ...
    def from_region_graph(  # type: ignore[misc]   # pylint: disable=too-many-arguments
        ...
        *,
        ...
        """Construct a folded TensorizedSMPC given a region graph, layers and hyper-parameters.
        ...
        Returns:
            TensorizedSMPC: A tensorized circuit.
        """
        ...
        input_layer_params = efamily_cls

        # Initialize input layer
        input_layer_pos = SMKernelPosImagLayer(num_vars=input_layer_params.num_vars, 
                                               num_output_units=input_layer_params.num_output_units, 
                                               params_module=input_layer_params)

        input_layer_neg = SMKernelNegImagLayer(num_vars=input_layer_params.num_vars, 
                                               num_output_units=input_layer_params.num_output_units, 
                                               params_module=input_layer_params)
        ...
        bookkeeping, inner_layers = TensorizedSMPC._build_layers(
            rg_layers,
            layer_cls,
        ...
        return cls(input_layer_params, input_layer_pos, input_layer_neg, scope_layer, bookkeeping, inner_layers)
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        """
        super().__init__()
        self.input_layer_params = input_layer_params
        self.input_layer_pos = input_layer_pos
        self.input_layer_neg = input_layer_neg
    ...
    def num_vars(self) -> int:
        ...
        return self.input_layer_pos.num_vars

    def __call__(self, x1: Tensor, x2: Tensor) -> Tensor:
        """Invoke the forward function.

        Args:
            x1 (Tensor): The first input to the circuit kernel, shape (*B, D, C).
            x2 (Tensor): The second input to the circuit kernel, shape (*B, D, C).
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        Args:
            x1 (Tensor): The first input to the circuit, shape (*B, D, C).
            x2 (Tensor): The second input to the circuit, shape (*B, D, C).

        Returns:
            Tensor: The output of the circuit, shape (*B, K).
        """
        x_pos = self.input_layer_pos(x1, x2)  # shape (*B, D, K, P)
        x_neg = self.input_layer_neg(x1, x2)  # shape (*B, D, K, P)
        x_pos = self.scope_layer(x_pos)  # shape (F, K, *B)
        x_neg = self.scope_layer(x_neg)  # shape (F, K, *B)

        log_output_pos = self._eval_layers(x_pos).movedim(source=0, destination=-1)  # shape (K, *B) -> (*B, K)
        log_output_neg = self._eval_layers(x_neg).movedim(source=0, destination=-1)  # shape (K, *B) -> (*B, K)

        combined_output = ((torch.exp(log_output_pos) + torch.exp(log_output_neg)) * 0.5).real
