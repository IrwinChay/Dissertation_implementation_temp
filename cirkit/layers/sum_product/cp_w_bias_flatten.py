...
class BaseCPLayer(SumProductLayer):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        *,
        ...
        if params_in_dim_name:
            ...
            # "{fio},f{i}...->f{o}..."
            self._einsum_in = f"{params_in_dim_name},f{i}...->f{o}..."
            ...
            assert (fold_mask is None), "Fold mask should be None"
            # Only params_in can see the folds and need mask.
            # size = self._infer_shape(params_in_dim_name)
            # self.params_in = torch.nn.Parameter(torch.zeros(size))
    ...
    def _forward_in_linear(self, x: Tensor) -> Tensor:
        ...
        ## # shape (F, K_in, *B) -> (F, H, K_out, *B)
        # "{fio},f{i}...->f{o}..."
        # shape (F, H, K, *B) -> (F, H, K, *B)
        return torch.einsum(self._einsum_in, params, x)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        # x = torch.exp(self._forward_reduce_log(x))  # shape (F, K, *B)
        x = torch.exp(torch.sum(x, dim=1))
        # x = torch.exp(torch.sum(x, dim=1))
        x = torch.einsum(self._einsum_in, self.params_in(), x)
        # if self.params_in is not None:
        #     # x = log_func_exp(
        #     #     x, func=self._forward_in_linear, dim=2, keepdim=True
        #     # )  # shape (F, H, K, *B)
        #     x = self._forward_in_linear(x)
...
def CPLayerWithBias(  # pylint: disable=invalid-name,too-many-arguments
    *,
    num_input_units: int,
    num_output_units: int,
    arity: int = 2,
