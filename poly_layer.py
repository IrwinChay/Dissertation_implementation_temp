...
class PolyLayer(nn.Module):
    ...
    def _make_parameters(self) -> None:
        ...
        if self.hidden:
            self.hidden1 = nn.Linear(self.in_features_dim, 50)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        input = F.relu(self.hidden1(input.to(dtype=torch.float32)))
        ...
        c = self.kernels_centers.expand(batch_size, self.num_kernels, 50)

        # print("input", input.dtype)
        # print("c", c.dtype)
        # kernel_inner = torch.bmm(input.to(dtype=torch.float32).view(
        #     batch_size, 1, self.in_features_dim), c.transpose(1, 2)) 
        kernel_inner = torch.bmm(input.to(dtype=torch.float32).view(
            batch_size, 1, 50), c.transpose(1, 2)) 
        # Output shape: (batch_size, 1, num_kernels)
        
        kernels = torch.pow(torch.squeeze(kernel_inner, dim=1), self.POWER_D) + self.bias
