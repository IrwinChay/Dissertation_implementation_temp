...
num_vars = 9
num_mixtures_s = [119, 458, 1798, 7127]
# [130, 500, 1967, 7802]
    ...
    for num_mixtures in num_mixtures_s:
        ...
        for run in range(1, 4):  # Running the model 3 times
            ...
            ds_train = torch.utils.data.TensorDataset(
                torch.from_numpy(x_train_real_normalized).float(),
                torch.from_numpy(y_train_real).float(),
            )
            ...
            metric = Loss(
                lambda y_pred, y: -likelihood.expected_log_prob(y, y_pred).mean()
            )
            ...
            if torch.cuda.is_available():
                ...
                print(
                    f"Run {run} - Total training time: {total_time:.2f} seconds, Peak GPU memory used: {peak_memory:.2f} MB"
                )
            else:
            ...
            def eval_step(engine, batch):
                ...
                with torch.no_grad():  # Disable gradient computation for evaluation
                    ...
                    y_pred = (
                        distribution.mean
                    )  # Use the mean of the distribution as the prediction
