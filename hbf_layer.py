...
class HBFLayer(nn.Module):
    """
    Defines a Hyper Basis Function Layer

    An HBF is defined by 5 elements:
    ...
    def _make_parameters(self) -> None:
        ...
        if self.constant_shape_parameter:
            ...
            self.log_shapes = nn.Parameter(
                torch.zeros(self.in_features_dim, self.num_kernels, dtype=torch.float32))
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # (B, D, K)
        c = self.kernels_centers.expand(batch_size, self.in_features_dim, self.num_kernels)

        diff = input.view(batch_size, self.in_features_dim, 1) - c

        # Apply norm function on a per-dimension basis; diff has size B x num_kernels x Fin
        # NOTE: No change needed here as we will apply the shape parameter per dimension next

        # Adjust shape parameter application to be per-dimension
        # eps_r now considers a width parameter per dimension for each kernel
        # log_shapes is reshaped to (1, num_kernels, in_features_dim) for broadcasting
        # (B, D, K)
        eps_r = diff * self.log_shapes.exp().view(1, self.in_features_dim, self.num_kernels)

        log_rbf = (-eps_r.pow(2))
        
        # (B, K)
        rbfs = torch.exp(torch.sum(log_rbf, dim=-2))
        
        
        # (B, 1, D)
        # log_rbf = torch.einsum('bkd,ki->bid', log_rbf, self.weights)

        # (B, K)
        # rbfs = torch.exp(torch.sum(log_rbf, dim=-1))
