...
class HBFLayer2(nn.Module):
    ...
    def _make_parameters(self) -> None:
        ...
        if self.constant_shape_parameter:
            ...
            self.log_shapes = nn.Parameter(
                torch.zeros(self.num_kernels, self.in_features_dim, self.in_features_dim, dtype=torch.float32))
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        cov = self.log_shapes.exp().float()
        
        eps_r = torch.einsum('bnd,kde,bne->bk', diff, cov, diff)

        # Apply norm function; c has size B x num_kernels
        # r = self.norm_function(eps_r)
