...
class SMKernel(Kernel):
    ...
    def forward(
        ...
        # (K, 1, 1, D)
        mus = self.mixture_means.view(self.num_mixtures, 1, 1, self.ard_num_dims)
        ...
        # (K, B, B)
        exps = torch.sum(exp_term, dim=-1).exp_()
        # (1, B, B, D) * (K, 1, 1, D) -> (K, B, B)
        cos_term = ((x1_cos_ - x2_cos_) * mus).sum(dim=-1).mul_(2 * math.pi)
        
        # (K, B, B)
        res = exps * cos_term.cos_()
        
        # Sum over mixtures   
        # (B, B)
        res = torch.einsum('k,kij->ij', self.mixture_weights, res)
        return res